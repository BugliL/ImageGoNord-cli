...
class ClientShould(UnitTestBaseClass):
    ...
    def test_return_docs_when_given_help_extended_parameter(self):
        ...
        self.assertTrue(
            "ImageGoNord, a converter for a rgb images to norththeme palette" in output
        )
    ...
    def test_no_output_when_convert_to_nord_palette_and_quiet_short_parameter_provided(
        self,
    ):
        _, output = run_image_go_nord_client(f"-i={self.input_image_path}", "-q")
        self.assertEqual("", output)

    def test_no_output_when_convert_to_nord_palette_and_quiet_extended_parameter_provided(
        self,
    ):
        _, output = run_image_go_nord_client(
            f"--img={self.input_image_path}", "--quiet"
        )
        self.assertEqual("", output)
    ...
    def test_convert_to_nord_palette_when_given_only_img_and_out_parameters_in_extended_version(
        self,
    ):
